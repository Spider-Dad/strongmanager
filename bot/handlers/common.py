...
async def cmd_help(message: types.Message, config):
    ...
    if is_admin:
        ...
        help_text = (
            ...
            f"  • Текущие настройки автосинхронизации\n\n"
            f"{bold('О системе алертов:')}\n"
            f"Доступны сводки по ошибкам через команду /alerts\\.\n\n"
...
async def cmd_about(message: types.Message, config):
    ...
    if is_admin:
        ...
        await message.answer(
            ...
            f"• Получение алертов об ошибках бота\n"
            f"• Управление синхронизацией БД\n"
            f"• Просмотр истории синхронизаций\n\n"
            f"{bold('Техническая информация о возможностях бота:')}\n"
